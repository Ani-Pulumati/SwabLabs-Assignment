...
c) cucumber.properties - this is used to set the cucumber related properties.

4. JRE - Java Runtime Environment details are stored here.

5Maven dependencies - all the jar files downloaded  from the maven repository are stored here.
...
To run the test cases user needs to select the feature file and run as cucumber feature.After the execution the cucumber report is generated which contains all the details of the status of the test case PASS and FAIL along with screen shots if the test case fails.


1st 5 test cases are automated from the TestSuite attached in the git hub.Data driven approach is used to test the 5 scenarios.
